...
const CalcScreen = (props) => {
  ...
  function putCursor() {
    const leftPos = 58.5 + 16.5 * (22 - edgeRight + cursorPos);
    const animate = currentLine === 0 ? 'blinker 1s step-start infinite' : 'none';
    const topPos = 44 + 35.1 * (props.rowsOnScreen - 1);
    return {
      left: leftPos + 'px',
      animation: animate,
      top: topPos + 'px',
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={'calc-number-column calc-number-column-'.concat(props.colorScheme)}
